      ...
      {({ open }) => (
        <>
          {isClickable ? (
            ...
            <MenuButton 
              className={
                `"flex inline-flex items-center px-4 py-2 text-xs font-semibold transition-all duration-200",
          ...
          {hasChildren && (
            ...
            >
              <MenuItems 
                className={
                  "absolute left-0 z-50 mt-1 w-56 origin-top-left rounded-sm bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none border-t-2 border-white"}
              >
                <div className="py-1">
                  {sortedChildren.map((child) => (
                    <MenuItem 
